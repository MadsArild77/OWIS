...
def main():
    ...
    parser.add_argument("--cases", type=Path, help="Locally labeled JSON cases; labels must precede model evaluation")
    parser.add_argument("--corpus", type=Path, help="Optional article list for candidate retrieval with distractors")
    ...
    cases=json.loads((args.cases or Path(__file__).parents[1]/"tests/fixtures/news_match_cases.json").read_text(encoding="utf-8"))
    articles={int(item['id']):item for case in cases for item in (case['a'],case['b'])}
    if args.corpus:
        for item in json.loads(args.corpus.read_text(encoding='utf-8')):
            articles.setdefault(int(item['id']), item)
    ...
    with tempfile.TemporaryDirectory() as tmp, patch.object(httpx.Client,"post",guarded):
        ...
        db.init_db()
        vectors=embed_articles(list(articles.values()))
        candidates=build_candidate_pairs(list(articles.values()),days_window=30,embeddings=vectors)
        candidate_ids={tuple(sorted((a['id'],b['id']))) for a,b,_ in candidates}
        rows=[]
        ai=AIClient()
        for index, case in enumerate(cases, 1):
            a,b=case["a"],case["b"]
            candidate=tuple(sorted((a['id'],b['id']))) in candidate_ids
            judged=judge_pair(ai,a,b,0)
            expected=case["expected"]
            retrieval_ok=candidate if expected in {"same_event","update"} else True
            rows.append(dict(name=case["name"], expected=expected, actual=judged["relationship"],
                             candidate=candidate, urls=[a.get('article_url'),b.get('article_url')], reason=judged.get("reason_short"), passed=retrieval_ok and judged["relationship"]==expected and not judged["fallback"]))
        ...
        report=dict(model=settings.AI_MODEL, cases=rows, passed=sum(r["passed"] for r in rows),
                    ...
                    corpus_size=len(articles), candidate_pairs=len(candidates),
